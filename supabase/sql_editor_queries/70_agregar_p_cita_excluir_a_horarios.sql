-- ============================================================
-- Agrega soporte para excluir la cita actual al reprogramar
-- Aplica a: obtener_grilla_horarios y obtener_horarios_disponibles
-- Ejecutar en Supabase SQL Editor
-- ============================================================

-- 1. obtener_grilla_horarios con p_cita_excluir
CREATE OR REPLACE FUNCTION public.obtener_grilla_horarios(
  p_sucursal_id  uuid,
  p_servicio_id  uuid,
  p_fecha        date,
  p_barbero_id   uuid,
  p_promocion_id uuid DEFAULT NULL::uuid,
  p_cita_excluir uuid DEFAULT NULL::uuid   -- ← NUEVO: cita actual en reprogramación
)
RETURNS TABLE(hora_inicio timestamp with time zone, libre boolean)
  ...
  select
    ...
    not exists (
      ...
      where c.barbero_id = p_barbero_id
        ...
        -- Si se está reprogramando, ignorar la cita actual del cliente
        and (p_cita_excluir is null or c.id <> p_cita_excluir)
...
-- 2. obtener_horarios_disponibles con p_cita_excluir
CREATE OR REPLACE FUNCTION public.obtener_horarios_disponibles(
  p_sucursal_id  uuid,
  p_servicio_id  uuid,
  p_fecha        date,
  p_barbero_id   uuid DEFAULT NULL::uuid,
  p_promocion_id uuid DEFAULT NULL::uuid,
  p_cita_excluir uuid DEFAULT NULL::uuid   -- ← NUEVO: cita actual en reprogramación
)
RETURNS TABLE(barbero_id uuid, hora_inicio timestamp with time zone)
LANGUAGE plpgsql STABLE SECURITY DEFINER
SET search_path TO 'public'
AS $$
declare
